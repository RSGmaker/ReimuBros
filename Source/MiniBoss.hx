...
class MiniBoss extends Enemy
{
	...
	public var maxdanmakutime:Int;
	public var danmakutime:Int;
	//public var unlock:String;
	
	
	public function new(bosstype:String="minibosslily_white",char:String="lily_white") 
	{
		super(bosstype);
		rename = char;
		subtype = "boss";
	...
	public function dodanmaku()
	{
		if (shottime > 0)
		{
			...
			if (shottime <= 0)
			{
				var O = new DanmakuBullet("bullet");
				O.x = cx;
				O.y = cy;
				O.Vspeed = 5;
				O.visuallyrotates = true;
				//O.changeangle(DegreeToRadian(180 + (rng.random() * 20)));
				//O.changeangle(DegreeToRadian(180 - (rng.random() * 90)));
				//O.changeangle(DegreeToRadian(120 - (rng.random() * 30)));
				O.changeangle(DegreeToRadian(120 - (rng.random() * 60)));
				O.shotby = this;
				...
				game.AddObject(O);
				/*O = new DanmakuBullet("bullet");
				O.x = cx;
				O.y = cy;
				O.Vspeed = 5;
				O.visuallyrotates = true;
				//O.changeangle(DegreeToRadian(0 - (rng.random() * 20)));
				//O.changeangle(DegreeToRadian(0 + (rng.random() * 90)));
				O.changeangle(DegreeToRadian(60 + (rng.random() * 30)));
				O.shotby = this;
	...
	public override function update()
	{
		if (!started)
		{
			...
			var T = Type.getClass(this);
			
			var i = 0;
			var ae = game.activeEnemies;
			while (i < ae.length)
			{
				var E = ae[i];
				if (E != this && E.charname == charname && E.alive)
				{
					alive = false;
					killed = true;
				}
		...
		cx = x + W2;
		cy = y + H2 + (H2 >> 1);
		//var done:Bool = (frame > 225);
		
		var done:Bool = (danmakutime > maxdanmakutime);
		if (done)
		{
			ty = -100;
		}
